...
pub fn add_targets(ctx: &mut Context, target_vec: &mut Vec<MovingEntity>) {
    let mut x_axis: CoordinateMovement =
        CoordinateMovement::new(0.0, 800.0, 0.0, 0.0, BoundaryBehavior::Bounce);
    let y_axis: CoordinateMovement =
        CoordinateMovement::new(0.0, 600.0, 300.0, 0.0, BoundaryBehavior::Bounce);
    for _ in 0..10 {
