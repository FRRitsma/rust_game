...
impl CoordinateMovement {
    pub fn new(min:f32, max:f32, position:f32, velocity:f32, boundary_behavior:BoundaryBehavior) -> Self{
        assert!(min < max);
        CoordinateMovement {
            min,
            max,
    ...
    pub fn update(&mut self){
        ...
        match self.get_boundary_behavior(){
            BoundaryBehavior::Wrap => {
                ...
                let new_position_offset = (position_offset + self.get_velocity()).rem_euclid(self.get_max() - self.get_min());
...
#[test]
fn test_update(){
    let mut coordinate_movement = CoordinateMovement::new(1.0, 3.0, 1.0, 2.0, BoundaryBehavior::Wrap);
...
fn test_bounces_at_zero_negative_velocity(){
    let mut movement_object = CoordinateMovement::new(0.0, 10.0, 3.0, -4.0, BoundaryBehavior::Bounce);
    movement_object.update();
...
fn test_bounces_at_offset_negative_velocity(){
    let mut movement_object = CoordinateMovement::new(1.0, 10.0, 3.0, -4.0, BoundaryBehavior::Bounce);
    movement_object.update();
...
fn test_bounces_at_positive_velocity(){
    let mut movement_object = CoordinateMovement::new(1.0, 10.0, 7.0, 4.0, BoundaryBehavior::Bounce);
    movement_object.update();
